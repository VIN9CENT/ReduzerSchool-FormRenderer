...
const faqs = [
  {
    ...
    answer: ["Reduzer School is a rigorous, one-year, fully in-person software engineering bootcamp based in Kisii, Kenya.",
  ...
  },
  {
    ...
    answer: ["We are fully in-person, not online or hybrid. We run for a full 52 weeks.", 
  ...
  },
  {
    ...
    answer: ["JavaScript, TypeScript, React, and Node.js, frontend, backend, and full-stack engineering using the exact tools global companies are actively hiring for right now.", 
  ...
  },
  {
    ...
    answer: ["We provide structured career support, portfolio review, CV and LinkedIn optimization, technical interview preparation, and access to opportunities through the Reduzer Technologies network.", 
  ...
  },
  {
    ...
    answer: ["Applicants may be high school graduates, university students, dropouts, or graduates who are serious about building a career in software engineering.", 
...
export default function FAQ() {
  ...
  return (
    <section id="faq" className="flex flex-col gap-8 bg-(--color-faq-white) opacity-95 text-black py-12">
      <h2 className="text-center font-extrabold text-4xl">FAQ</h2>
     {faqs.map((faq) => (
      <div key={faq.id} onClick={() => toggleFaq(faq.id)} className="shadow-none sm:shadow-3xl rounded-md bg-(--color-faq-white-1) cursor-pointer p-3 mx-auto w-[70%]">
        <div className="flex justify-between items-center">
        <h2 className="font-bold text-[14px] sm:text-[16px]">{faq.question}</h2>
        {activeId === faq.id ? <ChevronUp color="var(--color-red)" size={24}/> : <ChevronDown color="var(--color-red)" size={24}/>}
        </div>
        {activeId === faq.id && (
          <div className="mt-3 mx-6 flex flex-col gap-2">
